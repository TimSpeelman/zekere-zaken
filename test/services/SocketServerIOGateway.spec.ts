import { Result } from "../../src/services/IdentityGatewayInterface";
import { SocketServerIDGateway } from "../../src/services/SocketServerIDGateway";
...
describe("IDGateway", () => {
    ...
    it("connects", (_done) => {
        ...
        const gateway = new SocketServerIDGateway(agent);
    ...
    it("two peers can transfer a VerificationRequest using references (e.g. over QR)", (_done) => {
        ...
        Promise.all([
            ...
            subject.incomingVerifReqHook.on((inVer) => {
    ...
    it("two peers can execute a Verification", (_done) => {
        ...
        Promise.all([
            ...
            subject.completedVerifHook.on(({ subjectId, verifierId, request, result }) => {
                expect(verifierId).to.equal("VERIF");
                expect(subjectId).to.equal("SUBJ");
                expect(result).to.equal(Result.Succeeded);
                done();
            })
            verifier.completedVerifHook.on(({ subjectId, verifierId, request, result }) => {
                expect(verifierId).to.equal("VERIF");
                expect(subjectId).to.equal("SUBJ");
                expect(result).to.equal(Result.Succeeded);
                done();
    ...
    function makeAgent(peerId: string): MockAgent {
        ...
        return {
            ...
            verifyPeer: (peerId: string) => Promise.resolve(Result.Failed),
            ...
            mockVerify: (pId: string, req: IPv8VerifReq) => {
                if (!_handler) {
                    ...
                    return _handler(req).then(consent => Result.Succeeded);
                }
            }
        };
    }
    ...
    function mockPair(idA: string, idB: string) {
        ...
        const gatewayA = new SocketServerIDGateway(agentA);

        const agentB = makeAgent(idB);
        const gatewayB = new SocketServerIDGateway(agentB);
